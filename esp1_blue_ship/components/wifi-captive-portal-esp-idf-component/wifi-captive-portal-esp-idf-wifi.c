...
const char *captive_portal_wifi_tag = "wifi-captive-portal-esp-idf-wifi";

esp_event_loop_handle_t captive_portal_wifi_event_loop_handle;
static struct captive_portal_wifi_task_action_t wifi_task_action;

ESP_EVENT_DEFINE_BASE(CAPTIVE_PORTAL_WIFI_EVENT);
                               ...
                               int32_t event_id, void *event_data)
{
  if (event_id == WIFI_EVENT_AP_STACONNECTED)
  {
    ...
    ESP_LOGI(captive_portal_wifi_tag, "station " MACSTR " join, AID=%d",
             MAC2STR(event->mac), event->aid);
  }
  else if (event_id == WIFI_EVENT_AP_STADISCONNECTED)
  {
    wifi_event_ap_stadisconnected_t *event = (wifi_event_ap_stadisconnected_t *)event_data;
    ESP_LOGI(captive_portal_wifi_tag, "station " MACSTR " leave, AID=%d",
             MAC2STR(event->mac), event->aid);
  }
}

static void captive_portal_wifi_ap_init(void)
{
  ...
  ESP_LOGI(captive_portal_wifi_tag, "starting WiFi access point: SSID: %s password:%s channel: %d",
           CONFIG_EXAMPLE_WIFI_AP_SSID, CONFIG_EXAMPLE_WIFI_AP_PASSWORD, CONFIG_EXAMPLE_WIFI_AP_CHANNEL);

  /** Wifi captive portal DNS init. */
  captive_portal_dns_init();
}

static void captive_portal_wifi_init(void)
{
  /** Initialize NVS. */
...
void captive_portal_wifi_task(void *pvParameter)
{
  while (1)
  {
    while (1)
    {
      ...
      if (pvParameter != NULL)
      {
        wifi_task_action = CAPTIVE_PORTAL_WIFI_TASK_ACTION_COPY(pvParameter);
      }
      ...
      if (!wifi_is_init)
      {
        ESP_LOGI(captive_portal_wifi_tag, "wifi initializing...");

        /** Initialize wifi */
        captive_portal_wifi_init();

        wifi_is_init = true;
      }

      /** Send an event which says "this task is finished". */
      err = esp_event_post_to(captive_portal_wifi_event_loop_handle, CAPTIVE_PORTAL_WIFI_EVENT, CAPTIVE_PORTAL_WIFI_EVENT_FINISH, NULL, 0, portMAX_DELAY);
      if (err != ESP_OK)
      {
        ESP_LOGE(captive_portal_wifi_tag, "Sending event failed");
      }
