...
static void dns_recv(struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length)
{
  ...
  char buff[CAPTIVE_PORTAL_DNS_LEN];
  char reply[CAPTIVE_PORTAL_DNS_LEN];
  int i;
  ...
  // Some sanity checks:
  if (length > CAPTIVE_PORTAL_DNS_LEN)
  ...
  for (i = 0; i < my_ntohs(&hdr->qdcount); i++)
  {
    ...
    ESP_LOGI(DNS_TAG, "DNS: CAPTIVE_PORTAL_DNS_Q (type 0x%X cl 0x%X) for %s\n", my_ntohs(&qf->type), my_ntohs(&qf->cl), buff);

    if (my_ntohs(&qf->type) == CAPTIVE_PORTAL_DNS_QTYPE_A)
    {
      ...
      setn16(&rf->type, CAPTIVE_PORTAL_DNS_QTYPE_A);
      setn16(&rf->cl, CAPTIVE_PORTAL_DNS_QCLASS_IN);
      setn32(&rf->ttl, 0);
    ...
    else if (my_ntohs(&qf->type) == CAPTIVE_PORTAL_DNS_QTYPE_URI)
    {
      // Give uri to us
      rend = str_to_label(buff, rend, sizeof(reply) - (rend - reply)); // Add the label
      DnsResourceFooter *rf = (DnsResourceFooter *)rend;
      ...
      setn16(&rf->type, CAPTIVE_PORTAL_DNS_QTYPE_URI);
      setn16(&rf->cl, CAPTIVE_PORTAL_DNS_QCLASS_URI);
      setn16(&rf->ttl, 0);
...
static void dns_task(void *pvParameters)
{
  ...
  socklen_t fromlen;
  char udp_msg[CAPTIVE_PORTAL_DNS_LEN];
  ...
  while (1)
  {
    ...
    ret = recvfrom(sock_fd, (uint8_t *)udp_msg, CAPTIVE_PORTAL_DNS_LEN, 0, (struct sockaddr *)&from, (socklen_t *)&fromlen);
    if (ret > 0)
